...
assignmentsRouter.get("/client/:client_id", async (req, res) => {
  try {
    ...
    const data = await db.query(
      `SELECT * FROM assignments WHERE client_id = $1`,
      [client_id]
    );
    res.status(200).json(keysToCamel(data));
  } catch (err) {
...
assignmentsRouter.get("/search", async (req, res) => {
  ...
  try {
    const data = await db.query(
      `SELECT * FROM assignments WHERE event_id = $1 AND client_id = $2`,
      [event, client]
    );
    res.status(200).json(keysToCamel(data));
  } catch (err) {
...
assignmentsRouter.put("/:id", async (req, res) => {
  try {
    ...
    const { eventId, clientId, role } = req.body;
    const data = await db.query(
      `UPDATE assignments SET event_id = $1, client_id = $2, role = $3 WHERE id = $4 RETURNING *`,
      [eventId, clientId, role, id]
    );
    res.status(200).json(keysToCamel(data));
  } catch (err) {
    res.status(500).send(err.message);
